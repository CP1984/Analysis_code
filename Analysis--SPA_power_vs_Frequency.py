...
key_word = "GHz" # This keyword indicates the unit used in the file names
indep_var_label = "Frequency"
indep_var_unit = "Hz"
rescale_factor = 1e9 # 'rescale_factor' is used to adjust the magnitude of the independent variable. For example, a factor of 0.001 will convert a value of 1000 to 1.
...
for file_name in file_list:
    ...
    pattern = re.compile(r"(\d+p\d+)" + key_word)
    match = pattern.search(file_name)
    if match:
        matched_string = match.group(1)
        indep_var = float(matched_string.replace('p', '.'))
